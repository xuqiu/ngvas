...
class BaseShape {
    ...
    addEventListener(event, listener) {
        ...
        if (event === "click" || event === "dblclick") {
            this.canvas.addEventListener(event, this._eventHandlers[event] = (evt) => {
                let ep = this.eventPosition(evt,rect);
                if (this.isVisible && this.isHit(ep.x, ep.y)) {
                    listener(evt);
                }
            }, false);
        }
            ...
            if ("mousemove" in this._eventHandlers === false) {
                ...
                this.canvas.addEventListener("mousemove", this._eventHandlers.mousemove = (evt) => {
                    let ep = this.eventPosition(evt,rect);
                    const isHit = this.isVisible && this.isHit(ep.x, ep.y);
                    if (isOver && !isHit) {
    ...
    eventPosition(evt,rect){
        const clientX = evt.clientX + evt.currentTarget.offsetParent.scrollLeft - rect.left, clientY = evt.clientY + evt.currentTarget.offsetParent.scrollTop - rect.top;
